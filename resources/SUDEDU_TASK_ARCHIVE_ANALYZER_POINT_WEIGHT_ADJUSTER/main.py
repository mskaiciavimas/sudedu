...
class TaskPointsAnalyzer:
    def __init__(self, root):
        ...
        self.click_handler_cid = None  # Store click handler connection ID
    ...
    def setup_control_panel(self, parent):
        ...
        # Class selection - now includes "Overview"
        ...
        # Buttons - removed Reset Weights and Export Decoded Tasks
        ...
        self.info_label.grid(row=2, column=0, columnspan=7, pady=(10, 0))

        # Task info display area (NEW)
        task_info_frame = ttk.LabelFrame(parent, text="Task Information (Click on a bar)", padding="10")
        task_info_frame.grid(row=3, column=0, columnspan=7, pady=(10, 0), sticky=(tk.W, tk.E))
    ...
    def setup_graph_area(self, parent):
        """Setup the graph display area with 4 subplots (1 top + 3 bottom)."""
        # If figure already exists, close it
        if hasattr(self, 'fig'):
            plt.close(self.fig)
        ...
        # Top row: 1 full-width graph (spanning all 3 columns)
        self.ax_top = self.fig.add_subplot(gs[0, :])

        # Bottom row: 3 graphs
        self.ax_bottom_1 = self.fig.add_subplot(gs[1, 0])
        self.ax_bottom_2 = self.fig.add_subplot(gs[1, 1])
        self.ax_bottom_3 = self.fig.add_subplot(gs[1, 2])

        # Store axes for easy access
        self.axes = [self.ax_top, self.ax_bottom_1, self.ax_bottom_2, self.ax_bottom_3]
    ...
    def add_click_handler(self):
        ...
        if self.click_handler_cid is not None:
            self.canvas.mpl_disconnect(self.click_handler_cid)

        def on_click(event):
            # Check all axes that might have clickable bars
            valid_axes = self.axes
        ...
        self.click_handler_cid = self.canvas.mpl_connect("button_press_event", on_click)
    ...
    def setup_weights_table(self):
        ...
        self.weights_window.protocol("WM_DELETE_WINDOW", self.weights_window.withdraw)
        ...
        # Display current editing context - no period selector
    ...
    def on_class_changed(self, event=None):
        ...
        if selected == "Overview":
            ...
            self.current_class = int(selected)
        self.update_all_displays()
    ...
    def on_weight_period_changed(self, event=None):
        """Handle period change in weights editor - now just updates table."""
        self.update_weights_table()

    def update_all_displays(self):
        """Update graphs and calculations."""
        try:
            if self.class_var.get() != "Overview":
            ...
            if self.class_var.get() == "Overview":
                self.info_label.config(text="Viewing Overview")
            else:
    ...
    def update_graphs(self):
        """Update all graphs based on current selection."""
        try:
            # NUCLEAR OPTION: Completely recreate the graph area
            parent = self.canvas.get_tk_widget().master
            self.setup_graph_area(parent)

            # Reset bars for hover functionality
            self.hover_bars = {}

            contrib_period = self.contrib_period_var.get()

            # Check if Overview is selected
            is_overview = self.class_var.get() == "Overview"

            if is_overview:
                # Show only summary graph (top row)
                self.update_summary_graph(self.ax_top)
                # Hide bottom row graphs
                self.ax_bottom_1.set_visible(False)
                self.ax_bottom_2.set_visible(False)
                self.ax_bottom_3.set_visible(False)
                # Clear the text area
                self.task_info_text.config(state=tk.NORMAL)
                self.task_info_text.delete('1.0', tk.END)
                self.task_info_text.insert('1.0', 'Click on any bar in the graphs to see detailed task information...')
                ...
                # Show selected class graphs
                # Show top graph for selected period
                self.update_top_period_graph(self.ax_top, contrib_period)

                # Task Contribution to Total Points
                self.update_contribution_graph(self.ax_bottom_1, contrib_period)

                # Task Contribution per User
                self.update_contribution_per_user_graph(self.ax_bottom_2, contrib_period)

                # Task Frequency
                self.update_task_frequency_graph(self.ax_bottom_3, contrib_period)

            # Re-add click handler after updating graphs
    ...
    def filter_active_students(self, df, min_absolute=6, relative_factor=0.2):
        """Filter to only include students with sufficient task activity."""
        if len(df) == 0:
            return df, 0

        task_counts_per_student = df.groupby('studentId').size()

        # Calculate threshold
        median_tasks = task_counts_per_student.median()
        relative_threshold = median_tasks * relative_factor
        threshold = max(min_absolute, relative_threshold)

        # Filter
        active_students = task_counts_per_student[task_counts_per_student >= threshold].index
        filtered_df = df[df['studentId'].isin(active_students)]

        n_original = len(task_counts_per_student)
        n_filtered = len(active_students)
        print(f"Filtered {n_original} students -> {n_filtered} active students (threshold: {threshold:.1f} tasks)")

        return filtered_df, threshold

    def update_summary_graph(self, ax):
        """Update summary graph showing daily and weekly earnings per class."""
        all_df = self.df.copy()
        ...
        for user_class in sorted(all_df['userClass'].dropna().unique()):
            ...
            class_subset, threshold = self.filter_active_students(class_subset)
        ...
        if len(class_stats) > 0:
            ...
            ax.set_title(
                f"Summary: Daily & Weekly Earnings per User by Class (Active Students)\n(Weekly: from first entry, weeks with 3+ days)",
    ...
    def update_top_period_graph(self, ax, period):
        """Update top graph showing average daily points per task per user for selected period."""
        # Completely clear the axis
        ax.clear()
        ax.cla()
        ...
        if len(period_df) == 0:
            ax.text(0.5, 0.5, f'No data for Period {period}',
        ...
        bars = ax.bar(x, avgs, width=0.8, color=bar_color, alpha=0.8,
                      edgecolor='black', linewidth=1, label='Mean')

        # Initialize hover_bars for this axis if not exists
        if ax not in self.hover_bars:
            self.hover_bars[ax] = []
        ...
        ax.set_title(
            f"Average Daily Points per Task per User\nClass {self.current_class} | Period {period} ({self.get_period_label(period)})",
            fontsize=14, fontweight='bold')
        ax.legend(fontsize=10, loc='upper right')
        ax.grid(axis='y', alpha=0.3, linestyle='--')
        ax.set_ylim(bottom=0)

    def update_contribution_graph(self, ax, period):
        """Update graph showing task contribution to total points (bottom left)."""
        class_df = self.df[self.df['userClass'] == self.current_class].copy()
        contrib_df = class_df[class_df['period'] == period].copy()
        ...
        if len(contrib_df) > 0:
            ...
            if total_all_points > 0:
                ...
                ax.set_title(
                    f"Task Contribution to Total Points\nClass {self.current_class} | Period {period}",
                    fontsize=14, fontweight='bold')
    ...
    def update_contribution_per_user_graph(self, ax, period):
        """Update graph showing task contribution to total points per user (bottom middle)."""
        class_df = self.df[self.df['userClass'] == self.current_class].copy()
        contrib_df = class_df[class_df['period'] == period].copy()

        if len(contrib_df) == 0:
            ax.text(0.5, 0.5, f'No data for period {period}', ha='center', va='center', fontsize=12)
            return

        # Filter active students
        contrib_df, threshold = self.filter_active_students(contrib_df)

        if len(contrib_df) == 0:
            ax.text(0.5, 0.5, f'No active students in period {period}', ha='center', va='center', fontsize=12)
            return

        # Calculate contribution per user for each task
        user_task_stats = {}
        all_students = contrib_df['studentId'].unique()

        for student_id in contrib_df['studentId'].unique():
            user_data = contrib_df[contrib_df['studentId'] == student_id]
            user_total = user_data['calculated_points'].sum()

            if user_total > 0:
                for task_id in user_data['taskLibraryId'].unique():
                    task_data = user_data[user_data['taskLibraryId'] == task_id]
                    task_total = task_data['calculated_points'].sum()
                    contribution_pct = (task_total / user_total) * 100

                    if task_id not in user_task_stats:
                        user_task_stats[task_id] = []
                    user_task_stats[task_id].append(contribution_pct)

        # Calculate mean and median for each task
        # Calculate mean and median for each task
        task_summary = []
        for task_id, contributions in user_task_stats.items():
            median_val = np.median(contributions)
            participation_weight = len(contributions) / len(all_students)
            weighted_median = median_val * participation_weight
            ...
            task_summary.append({
                ...
                'std': np.std(contributions) if len(contributions) > 1 else 0,
                'n': len(contributions)
            })

        # Sort by weighted median descending
        task_summary = sorted(task_summary, key=lambda x: x['weighted_median'], reverse=True)

        # Filter: only show tasks done by at least 5% of students OR 3 students minimum
        min_student_percentage = 0.05
        min_absolute = 3
        min_students = max(min_absolute, int(len(all_students) * min_student_percentage))
        task_summary = [t for t in task_summary if t['n'] >= min_students]

        # Take top 20
        top_n = min(20, len(task_summary))
        task_summary = task_summary[:top_n]

        if len(task_summary) == 0:
            ax.text(0.5, 0.5, 'No task contribution data', ha='center', va='center', fontsize=12)
            return

        task_ids = [t['task_id'] for t in task_summary]
        means = [t['mean'] for t in task_summary]
        medians = [t['weighted_median'] for t in task_summary]
        n_students_list = [t['n'] for t in task_summary]

        # Get decoded task info
        decoded_tasks = []
        for task_id in task_ids:
            task_info_sample = contrib_df[contrib_df['taskLibraryId'] == task_id]['taskInfo'].iloc[0]
            decoded_task = self.decode_task_info(task_info_sample)
        ...
        # Create horizontal bars with mean and median combined
        colors = plt.cm.viridis(np.linspace(0.4, 0.9, len(task_ids)))
        bars = ax.barh(y, means, color=colors, edgecolor='black', linewidth=0.5, alpha=0.8)

        # Store bar data for hover
        self.hover_bars[ax] = []
        for i, (bar, task_id, mean_val, median_val, decoded) in enumerate(
                zip(bars, task_ids, means, medians, decoded_tasks)):
            self.hover_bars[ax].append({
                'bar': bar,
                'text': f"Task ID: {task_id}\nMean: {mean_val:.2f}%\nMedian: {median_val:.2f}%\n{decoded}"
            })

        # Add median markers on bars
        ax.scatter(medians, y, color='white', marker='D', s=50, zorder=5, edgecolor='black', linewidth=1)

        # Add text labels
        for i, (task_id, mean_val, median_val, n_students) in enumerate(
                zip(task_ids, means, medians, [t['n'] for t in task_summary[:top_n]])):
            participation_pct = (n_students / len(all_students)) * 100
            ax.text(mean_val + 0.5, i, f'M:{median_val:.1f}% ({participation_pct:.0f}%)',
                    va='center', fontsize=8, color='black')

        ax.set_yticks(y)
        ax.set_yticklabels(task_ids, fontsize=10)
        ax.set_xlabel('% of Total Points per User', fontsize=12)
        ax.set_ylabel('Task ID', fontsize=12)
        ax.set_title(
            f"Task Contribution to Total Points per User (Active: ≥{threshold:.0f} tasks)\nClass {self.current_class} | Period {period}",
            fontsize=14, fontweight='bold')
        ax.grid(axis='x', alpha=0.3, linestyle='--')
        ax.invert_yaxis()
        ax.set_xlim(left=0)

    def update_task_frequency_graph(self, ax, period):
        """Update graph showing task frequency per user (bottom right)."""
        class_df = self.df[self.df['userClass'] == self.current_class].copy()
        contrib_df = class_df[class_df['period'] == period].copy()
        ...
        # Calculate task frequency per user
        user_task_counts = {}
        all_students = contrib_df['studentId'].unique()

        for student_id in contrib_df['studentId'].unique():
            user_data = contrib_df[contrib_df['studentId'] == student_id]
            user_total_tasks = len(user_data)

            if user_total_tasks > 0:
                for task_id in user_data['taskLibraryId'].unique():
                    task_count = len(user_data[user_data['taskLibraryId'] == task_id])
                    frequency_pct = (task_count / user_total_tasks) * 100

                    if task_id not in user_task_counts:
                        user_task_counts[task_id] = []
                    user_task_counts[task_id].append(frequency_pct)

        # Calculate mean and median for each task
        task_summary = []
        for task_id, frequencies in user_task_counts.items():
            median_val = np.median(frequencies)
            participation_weight = len(frequencies) / len(all_students)
            weighted_median = median_val * participation_weight
            ...
            task_summary.append({
                ...
                'median': median_val,
                'weighted_median': weighted_median,
                'std': np.std(frequencies) if len(frequencies) > 1 else 0,
                'n': len(frequencies)
            })
        ...
        if len(task_summary) == 0:
            ax.text(0.5, 0.5, 'No task frequency data', ha='center', va='center', fontsize=12)
            return
        ...
        # Create horizontal bars with mean and median combined
        colors = plt.cm.plasma(np.linspace(0.3, 0.8, len(task_ids)))
        bars = ax.barh(y, means, color=colors, edgecolor='black', linewidth=0.5, alpha=0.8)
        ...
        # Add text labels with participation rate
        for i, (task_id, mean_val, median_val, t) in enumerate(zip(task_ids, means, medians, task_summary)):
            n_students = t['n']
            participation_pct = (n_students / len(all_students)) * 100
            ax.text(mean_val + 0.5, i, f'M:{median_val:.1f}% ({participation_pct:.0f}%)',
                    va='center', fontsize=8, color='black')
        ...
        ax.set_yticks(y)
        ax.set_yticklabels(task_ids, fontsize=10)
        ax.set_xlabel('% of Total Tasks per User', fontsize=12)
        ax.set_ylabel('Task ID', fontsize=12)
        ax.set_title(
            f"Task Frequency per User (Active: ≥{threshold:.0f} tasks)\nClass {self.current_class} | Period {period}",
            fontsize=14, fontweight='bold')
        ax.grid(axis='x', alpha=0.3, linestyle='--')
        ax.invert_yaxis()
        ax.set_xlim(left=0)
    ...
    def update_weights_table(self):
        """Update the weights table for selected class and contribution period."""
        current_period = self.contrib_period_var.get()

        self.weights_context_label.config(
            text=f"Editing: Class {self.current_class} | Period {current_period}"
        )
    ...
    def reset_all_weights(self):
        """Reset all weights - kept for internal use."""
